...
class HeightMap:
    ...
    def heights(self):
        ...
        padding_row = [[0] * (w + 1)]
        _heights = [flat_heights[j * w:(j + 1) * w] + [0] for j in range(h)] + padding_row
        return _heights
...
class Alm2(RageOfMages2Alm, HeightMap):
    ...
    @cached_property
    def walkable(self):
        _walkable = [[False] * self.width for _ in range(self.height)]
        for idx, tile in enumerate(self["tiles"].body.tiles):
            x, y = idx % self.width, idx // self.width
            terrain_column_id = self.tilecoords[y][x][0]
            # column_id = [28..31] - rock tiles
            # column_id = [32..48?] - water tiles
            ROCK_START = 28
            WATER_END = 47
            _walkable[y][x] = ROCK_START < terrain_column_id < WATER_END
            # terrain_id = tile.tile_id >> 8
            # _walkable[y][x] = terrain_id < 3
        return _walkable
