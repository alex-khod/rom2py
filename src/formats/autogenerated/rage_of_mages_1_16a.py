...
class RageOfMages116a(KaitaiStruct):
    ...
    def _read(self):
        if self.has_palette != 0:
            self.inner_palette = [None] * (256)
            for i in range(256):
                self.inner_palette[i] = self._io.read_u4le()


        self.sprite_records = [None] * (self.sprite_count)
        for i in range(self.sprite_count):
            self.sprite_records[i] = RageOfMages116a.SpriteRecord(self._io, self, self._root)
